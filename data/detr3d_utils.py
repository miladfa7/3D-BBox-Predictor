...
def load_depth_points_mat(depth_filename):
    ...
    # depth = np.fromfile(depth_filename, dtype=np.float32)
    depth = np.load(depth_filename)
    depth = depth.reshape(3, -1).T
    return depth
...
def my_compute_box_3dasd(center, size, heading_angle):
...
def compute_box_3dqwe(obj, calib):
...
def compute_orientation_3dqe(obj, calib):
    ''' Takes an object and a projection matrix (P) and projects the 3d
...
def draw_projected_box3dqe(image, qs, color=(255,255,255), thickness=2):
