...
MEAN_COLOR_RGB = np.array([0.5, 0.5, 0.5])  # sereact color is in 0~1
DATA_PATH_V1 = "" ## Replace with path to dataset
DATA_PATH_V2 = "" ## Not used in the codebase.


class SereactDatasetConfig(object):
    def __init__(self):
        self.num_semcls = 1
    ...
    def my_compute_box_3d_old(self, center, size, heading_angle):
    ...
    def my_compute_box_3d(self, center, size, heading_angle, origin=(0.5, 1.0, 0.5), axis=1):
        dims = size
        centers = center
...
class SereactDetectionDataset(Dataset):
    def __init__(
        self,
        dataset_config,
        split_set="train",
        ...
        random_cuboid_min_points=100000,
    ):
        ...
        self.data_path = f"sereact_trainval/sereact_pc_bbox_votes_50k_v2_{split_set}"    
    ...
    def __getitem__(self, idx):
        ...
        point_cloud = np.load(scan_path + "pc.npz")["pc"]  # Nx6
        bboxes = np.load(scan_path + "bbox.npy")  # K,8
        ...
        angle = bboxes[:, 6]
        size = bboxes[:, 3:6]
        corners_3d = self.dataset_config.my_compute_box_3d(center, size, angle)
        for i in range(corners_3d.shape[0]):
            # compute axis aligned box
            angle = 0
            angle_class, angle_residual = self.dataset_config.angle2class(angle)
            angle_classes[i] = angle_class
            angle_residuals[i] = angle_residual
            raw_angles[i] = angle % 2 * np.pi
            box3d_size = bboxes[i, 3:6] * 2
            raw_sizes[i, :] = box3d_size
            corner_3d = corners_3d[i]
            xmin = np.min(corner_3d[:, 0])
            ymin = np.min(corner_3d[:, 1])
            zmin = np.min(corner_3d[:, 2])
            xmax = np.max(corner_3d[:, 0])
            ymax = np.max(corner_3d[:, 1])
            zmax = np.max(corner_3d[:, 2])
