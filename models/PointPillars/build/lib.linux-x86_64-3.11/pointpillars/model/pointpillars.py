...
class Neck(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(outs)):
            if outs[i].shape[2:] != outs[0].shape[2:]:
...
class PointPillars(nn.Module):
    def __init__(self,
        ...
        ranges = [
            [0, -0.94, -0.6, 2.80, 2.80, -0.6],
            [0, -0.94, -0.6, 2.80, 2.80, -0.6],
            [0, -0.94, -1.78, 2.80, 2.80, -1.78]
        ]
        # sizes = [[0.6, 0.8, 1.73], [0.6, 1.76, 1.73], [1.6, 3.9, 1.56]]
        sizes = [
            [0.4, 0.6, 1.5],   # Smaller anchor for smaller objects
            [0.5, 1.0, 1.5],   # Medium-sized anchor
            [1.0, 2.0, 1.4]    # Larger anchor for bigger objects
        ]
        rotations=[0, 0]
        # rotations=[0, 1.57]
